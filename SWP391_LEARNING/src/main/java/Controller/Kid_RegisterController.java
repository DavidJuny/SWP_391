              ...
              try (PrintWriter out = response.getWriter()) {
                     ...
                     if (!kpassword.equals(kconfpassword)) {
                            ...
                            if (user == null) { //continue to signup
                                   try {
                                          kid newkid = kidDAO.registerk(parentID, kfullname, kusername, kpassword, kbirthday, kimage);
                                          //insert vao table KidLearning co status Lock cho 3 course C1, C2, C3
                                          proDAO.addLearningKid1(newkid.getKidID());
                                          proDAO.addLearningKid2(newkid.getKidID());
                                          proDAO.addLearningKid3(newkid.getKidID());
